...
ls /dev/serial/by-id
```
If you have only one esp32 connected to it, you can copy the only file that is listed
under this directory. The esp32 must be connected via the USB port that is marked UART.
Add the serial device to your sitl command by adding this:
````
